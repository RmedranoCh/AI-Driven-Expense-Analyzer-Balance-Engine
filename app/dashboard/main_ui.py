...
class ExpenseDashboard:
    def __init__(self):
        ...
        self._load_budget_topes()

    def _load_budget_topes(self):
        st.session_state.topes_presupuesto = {}
        with get_session() as db:
            for row in db.query(DBPresupuestoTope).filter(
                DBPresupuestoTope.user_id == self.user_id
            ).all():
                st.session_state.topes_presupuesto[row.categoria] = float(row.tope_mensual)
        for cat in self.classifier.categorias_validas:
            if cat not in st.session_state.topes_presupuesto:
                st.session_state.topes_presupuesto[cat] = 1000.0

    def _save_budget_topes(self):
        with get_session() as db:
            db.query(DBPresupuestoTope).filter(
                DBPresupuestoTope.user_id == self.user_id
            ).delete()
            for cat, tope in st.session_state.topes_presupuesto.items():
                db.add(DBPresupuestoTope(
    ...
    def render(self):
        ...
        btn_disabled = (not is_admin and remaining <= 0) or processing or uploaded_file is None
    ...
    def _process_file(self, file, is_admin=False):
        ...
        if file_hash in st.session_state.processed_hashes:
            st.warning("⚠️ Esta factura ya fue analizada en esta sesión. Si es otro archivo, recarga la página e inténtalo de nuevo.")
            return
        st.session_state.processed_hashes.add(file_hash)

        with get_session() as db:
            existente = db.query(DBGasto).filter(
                DBGasto.user_id == self.user_id,
                DBGasto.file_hash == file_hash
            ).first()
            if existente:
                st.warning(f"⚠️ Esta factura ya fue registrada anteriormente como '{existente.proveedor}'.")
                return
    ...
    def _render_confirmation_step(self):
        ...
        if col2.button("❌ Cancelar"):
            ...
            if file_hash and file_hash in st.session_state.processed_hashes:
        ...
        with col_config:
            ...
            edited_topes_df = st.data_editor(
                ...
                column_config={
                    ...
                    "Tope Mensual ($)": st.column_config.NumberColumn("Tope Mensual ($)", min_value=0.0, format="$%.2f", step=10.0)
                }
            )
            ...
            for _, row in edited_topes_df.iterrows():
                ...
                if nuevo != st.session_state.topes_presupuesto.get(row["Categoría"]):
                    hubo_cambio = True
                st.session_state.topes_presupuesto[row["Categoría"]] = nuevo

            if hubo_cambio:
                self._save_budget_topes()
